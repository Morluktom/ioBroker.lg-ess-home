...
const MAPPING ={
    ...
    backup_setting: {map: 'BackupMode'},
    winter_setting: {map: 'WinterMode'},
    alg_setting: {map: 'ChargingMode'},
    backup_soc: {map: 'BackupSoc'},
    auto_charge: {map: 'AutoCharge'},
    ...
    BackupSoc: {type: 'number', write: true, urlCmd: '/v1/user/setting/batt', cmdKey: 'backup_soc',
        unit: '%'
    },
    ...
    AutoCharge: {type: 'number', write: true, urlCmd: '/v1/user/setting/batt', cmdKey: 'autocharge',
        states: {
            0: 'off',
            1: 'on'},
        valueMap:{
    ...
    a_phase_1: {scaling: 0.1},
    a_phase_2: {scaling: 0.1},
    a_phase_3: {scaling: 0.1},
    pv_voltage: {scaling: 0.1},
    pv_current: {scaling: 0.01, unit: 'A'},
    sum_of_pv_capacity: {unit: 'kWp', scaling: 0.001}
};
...
function LgEss(adapter, ip, password, refreshTimeHome, refreshTimeCommon, instLogInPossible, installerpassword){
	 ...
	 ******************************************************************************************************************************/
    function Login(installerLogIn = false) {
                ...
                .then(response => {
                    ...
                    if (response.data.status == 'success'){
                        ...
                        if (response.data.status == 'password mismatched'){
                            if (response && response.config && response.config.url && response.config.url.indexOf('/user/') !== -1)
    ...
    async function TimerCommon() {
        try {
            const common =  ReadFromInverter(STATE_URL.Common);
	 ...
	 * @param {object} extra_json_data extra json data
	******************************************************************************************************************************/
    ...
    async function ParseData(data , path){
        try {
            ...
            await ParseDataRecursive(strName,data);
        } catch(err){
            adapter.log.error(`[LG ESS] (ParseData):  ${err}`);
        }
    }
    async function ParseDataRecursive(strPfad, data){
        try {
            const indexOfDot = strPfad.lastIndexOf('.');
            const strlength = strPfad.length;
            const strVarName = strPfad.substring(indexOfDot + 1, strlength);

            //------ Value is a Array ------
            if (Array.isArray(data)){
                for ( let i=0; i < data.length; i++ ) {
                    const strIndex = `${(i+1).toString()}`;
                    if (((Array.isArray(data[i]))) || (typeof(data[i]) == 'object')){
                        /* Create Channel */
                        await adapter.setObjectNotExistsAsync(`${strPfad}.${strIndex}`, {
                            type: 'channel',
                            common: {
                                name: `${strVarName}_${strIndex}`,
                            },
                            native: {}
                        });
                        await ParseDataRecursive(`${strPfad}.${strIndex}`,data[i]);
                    } else {
                        await WriteData(strPfad, `${strVarName}_${strIndex}`, data[i], false);
                        //await WriteData(`${strPfad}.${strIndex}`, `${strVarName}_${strIndex}`, data[i], false);
                    }
                }
            //------ Value is a Object ------
                ...
                for (const [variable, value] of Object.entries(data)) {
                    if ((Array.isArray(value)) || (typeof(value) == 'object')){
                        /* Create Channel */
                        await adapter.setObjectNotExistsAsync(`${strPfad}.${variable}`, {
                            type: 'channel',
                            common: {
                                name: variable,
                            },
                        ...
                        await ParseDataRecursive(`${strPfad}.${variable}`,value);
                    } else {
                        await WriteData(strPfad, variable, value, false);
                    }
                };
            } else {
                //await WriteData(strPfad, variable, data, false);
            }
        } catch(err){
            adapter.log.error(`[LG ESS] (ParseDataRecursive):  ${err}`);
        }
    }
	 ...
	 ******************************************************************************************************************************/
	 async function WriteData(path, variable, value, onlyUpdateDifferent){
        ...
        try {
            ...
            if (!obj){
                adapter.log.debug(`[LG ESS] CreateDataPoint Path ${path} Name: ${variable} Value: ${value}`);
	 ...
	 ******************************************************************************************************************************/
	 async function CreateDataPoint(path, variable, value){
        try {
            ...
            WriteValue(path, variable, r.type,  value, false);
        } catch(err){
    ...
    function GetTypeRollUnitScaling(path, variable, value){
        ...
        //Home ESS 15 plus
        if ((variable == 'capacity') && (path.startsWith('user.essinfo.common.ess.'))) variable = 'pv_capacity';
        if ((variable == 'voltage') && (path.startsWith('user.essinfo.common.ess.'))) variable = 'pv_voltage';
        if ((variable == 'current') && (path.startsWith('user.essinfo.common.ess.'))) variable = 'pv_current';
        if ((variable == 'status') && (path.startsWith('user.essinfo.common.ess.'))) variable = 'batt_status';
    ...
    this.SetCommand = function(key, val){
        ...
        else if (variable == 'BackupSoc')
            extra_json_data[MAPPING[variable].cmdKey] = val;
        else if (variable == 'AutoCharge')
            extra_json_data[MAPPING[variable].cmdKey] = val.toString();
        else {
            if (!MAPPING[variable].states || !MAPPING[variable].states[val]) return;
