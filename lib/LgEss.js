...
function LgEss(adapter, ip, password, refreshTimeHome, refreshTimeCommon, instLogInPossible, installerpassword){
	...
	var _timerCommon = null;
	var _timerHome = null;
	var _timerSlow = null;
	var _timerLogIn = null;
		...
		try {
			for (let i = 0; i < GRAPH_DEVICES.length; i++) {
				for (let u = 0; u < GRAPH_TIMESPANS.length; u++) {
					let date = formatTime(new Date(),GRAPH_TFORMATS[GRAPH_PARAMS[GRAPH_TIMESPANS[u]]]);
					let extra_json_data={};
					extra_json_data[GRAPH_PARAMS[GRAPH_TIMESPANS[u]]] = date;
					await ReadFromInverter(`/v1/user/graph/${GRAPH_DEVICES[i]}/${GRAPH_TIMESPANS[u]}`,extra_json_data);
				}
			}
		} catch (error) {
	 ...
	 ******************************************************************************************************************************/	
	 async function CreateDataPoint(path, variable, value){
		try {
			...
			var r = GetTypeRollUnitScaling(path,variable,value);
			let write = false;
