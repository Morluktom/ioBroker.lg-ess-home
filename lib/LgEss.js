...
function LgEss(adapter, ip, password, refreshTime, refreshTimeCommon){
		...
		for (const [variable, value] of Object.entries(data)) {
			if (typeof(value) == "object"){
				for (const [variable1, value1] of Object.entries(data[variable])) {
					...
					await this.writeData(strName + "." + variable, variable1, value1);
				};
			} else {
				this.writeData(strName, variable, value);
			}
		};	
	}

	this.writeData = async function(path, variable, value){
		...
		try {
			...
			if (!obj){
				...
				this.writeValue(path, variable, obj.common.type, value);
			}
		}
		catch (err){
	...
	this.writeValue = async function(path, variable, type, value){
		let id = path + "."+ variable;
		...
		if ((mapping[variable]) && (mapping[variable].map)){
			this.writeData("commands",mapping[variable].map,value);
		}
